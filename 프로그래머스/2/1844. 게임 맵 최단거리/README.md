...
직접 그려서 maps[0][0] 부터 '우측 > 하단 > 상단' 순으로 0인지, 1인지 탐색하는 방식을 구현했는데 BFS 접근방식은 아예 다름...
...
- 무한 루프 위험: while(true) 루프는 종료 조건 없이 계속 실행됩니다. 만약 상대 팀 진영에 도달할 수 없는 경우(예를 들어, 벽으로 완전히 둘러싸인 경우) 루프는 끝나지 않습니다.

- 방향성 문제: 제시한 코드는 오른쪽 또는 아래로만 이동할 수 있습니다. 즉, 필요한 경우 왼쪽이나 위로 이동하는 상황을 처리할 수 없습니다. 그러나 실제 최단 경로는 여러 방향으로의 이동을 포함할 수 있습니다.

- 이미 방문한 위치 재방문: 이미 방문한 위치를 체크하지 않기 때문에, 같은 위치를 반복해서 방문하는 문제가 발생할 수 있습니다. 이는 비효율적이며, 경우에 따라서는 무한 루프를 유발할 수도 있습니다.

- 경계 조건 처리 미흡: 배열의 경계를 넘어서는 접근(예: maps[i-1][j]가 i=0일 때)을 시도할 경우, 실행 시간 에러가 발생할 수 있습니다.
